...
class TestAdminModel:
    ...
    def test_formfield_for_foreignkey_sets_queryset(self) -> None:
        ...
        request.user = User.objects.create_superuser('Aryan', 'aryan@example.com', 'password')

        province: Province = Province.objects.create(name="Kermanshah", code=19)

        model_admin: IranianCitiesAdmin = IranianCitiesAdmin(model=City, admin_site=admin_site)

        db_field = City._meta.get_field('province')

        with patch.object(IranianCitiesAdmin, 'get_field_queryset', return_value=Province.objects.filter(name="Kermanshah")) as mock_get_field_queryset:
